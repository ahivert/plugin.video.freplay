...
def list_shows(channel, folder):
    ...
    if folder == 'none':
        match1 = re.compile(r'<ul data-role="listview" data-theme="a">(.*?)</ul>',
                            re.DOTALL).findall(html)
        if match1:
            match = re.compile(r'<li>(.*?)<a href="(.*?)">(.*?)</a>(.*?)</li>',
                               re.DOTALL).findall(match1[0])
            if match:
...
def list_videos(channel, show):
    ...
    match = re.compile(
        r'<li data-icon="false">(.*?)<a href="(.*?)">(.*?)<img src="(.*?)"/>(.*?)<p class="ui-li-aside">(.*?)</p>(.*?)<h3>(.*?)</h3>(.*?)<p>(.*?)</p>(.*?)</li>',
        re.DOTALL
    ).findall(html)
