...
def list_shows(channel, param):
    ...
    filePath = utils.downloadCatalog(
        'http://api.france24.com/fr/services/json-rpc/emission_list?databases=f24fr&key=XXX&start=0&limit=30&edition_limit=8',
        'France24.json', False, {}
    )
    ...
    for item in jsonParser['result']['f24fr']['list']:
        shows.append([
            channel, item['nid'], item['title'].encode('utf-8'),
            item['image'][0]['original'].encode('utf-8'), 'shows'
        ])
    ...
    for item in jsonParser['result']['f24fr']['list']:
        if item['nid'] == show:
            for video in item['editions']['list']:
                url = video['video'][0][globalvar.ADDON.getSetting('%sQuality' %
