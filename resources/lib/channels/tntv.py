...
def list_videos(channel, show):
    ...
    filePath = utils.downloadCatalog(
        'http://tntv.goodbarber.com/apiv3/getItemsByCategorie/6833037/%s/1/%s/' % (show, nbItems),
        'tntv%s.json' % (show), False, {}
    )
...
def getVideoURL(channel, video_id):
    ...
    t = video_id.split('-')
    filePath = utils.downloadCatalog(
        'http://tntv.goodbarber.com/apiv3/getItemsByCategorie/6833037/%s/1/%s/' % (t[0], nbItems),
        'tntv%s.json' % (t[0]), False, {}
    )
