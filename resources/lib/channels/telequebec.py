...
def list_shows(channel, folder):
    shows = []
    filePath = utils.downloadCatalog(url_shows, 'telequebec.html', False,
                                     {})    # Page principale du replay
    ...
    if folder == 'none':
        ...
        for item in list_item:
            ...
            shows.append([
                channel, url_video + '|' + title, title + " - " + infos, url_img, 'shows'
            ])
    return shows
...
def list_videos(channel, params):
    ...
    filePath = utils.downloadCatalog(
        program_url, 'telequebec_' + titre_program + '.html', False, {}
    )
    html = open(filePath).read().decode("utf-8")
