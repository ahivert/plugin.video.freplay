...
def list_shows(channel, folder):
    ...
    if folder == 'none':
        ...
        for em in jsonParser['data']['results']:
            shows.append([
                channel, em['slug'], em['title'].encode('utf-8'), em['header']['url-list'], 'folder'
            ])
    else:
        filePath = utils.downloadCatalog(url_themes, 'VTELE.JSON', False, {})
        filPrgm = open(filePath).read()
...
def list_videos(channel, id):
    ...
    for ep in jsonParser['data']['results']:
        if 'id' in ep['episode']:
            ...
            infoLabels = {"Title": title, "Plot": description, 'Duration': duration}
...
def getVideoURL(channel, slug):
    ...
    filePath = utils.downloadCatalog(
        url_video_json % (da, vid), 'VTELE%s.json' % vid, True, {}, {
            'Accept':
            'application/json;pk=BCpkADawqM145C2ijKGLTnwjc7d61ZlONDwMyE_8xJ-8UVM6hjWtMzw5gMSTc3xniaSHQAJ7Mj23t6tajoj7IQdwFuqpwLac1Gm0L6F11zSTeSWfsOW2KF83_vo'
        }
    )
    filPrgm = open(filePath).read()
    print filPrgm
