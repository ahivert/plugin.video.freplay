...
def list_shows(channel, folder):
    ...
    if folder == 'none':
        match = re.compile(
            r'<div class="list-cat-item">(.*?)<a href="(.*?)">(.*?)<img src="(.*?)" alt=(.*?)<span class="list-cat-item-title">(.*?)</span>',
            re.DOTALL
        ).findall(html)
        if match:
...
def list_videos(channel, show):
    ...
    match = re.compile(
        r'<div class="video-list-item video-cat-list-item">(.*?)<a href="(.*?)">(.*?)alt="(.*?)" src="(.*?)">(.*?)<span class="video-list-item-info-duree">(.*?):(.*?)</span>',
        re.DOTALL
    ).findall(html)
