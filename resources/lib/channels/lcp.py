...
def parse_dict(dictText):
    infos = {}
    try:
        infos['Thumb'] = html_parser.unescape(
            re.findall('<key>thumbnail_big</key> +<string>(.+?)</string>', dictText)[0]
        ).encode('utf-8')
    except:
        infos['Thumb'] = html_parser.unescape(
            re.findall('<key>thumbnail_small</key> +<string>(.+?)</string>', dictText)[0]
        ).encode('utf-8')
    infos['Plot'] = BeautifulSoup(
        html_parser.unescape(
            re.findall('<key>full_description</key> +<string>(.+?)</string>', dictText)[0]
        )
    ).get_text().encode('utf-8')
    infos['PlotOutline'] = BeautifulSoup(
        html_parser.unescape(
            re.findall('<key>short_description</key> +<string>(.+?)</string>', dictText)[0]
        )
    ).get_text().encode('utf-8')
    title = BeautifulSoup(
        html_parser.unescape(re.findall('<key>title</key> +<string>(.+?)</string>', dictText)[0])
    ).get_text().encode('utf-8')
    url = html_parser.unescape(re.findall('<key>url</key> +<string>(.+?)</string>',
                                          dictText)[0]).encode('utf-8')
