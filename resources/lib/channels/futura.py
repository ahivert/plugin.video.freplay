...
def list_shows(channel, folder):
    ...
    filePath = utils.downloadCatalog(
        'http://www.futura-sciences.com/videos/', 'futuraMain.html', False, {}
    )
    ...
    match = re.compile(
        r'<a class="nav-sections-item-link" href="(.*?)"><h2 class="nav-sections-item-title text-shadow">(.*?)</h2><p class="nav-sections-item-count show-on-hover text-shadow">(.*?)</p></a></article>',
        re.DOTALL
    ).findall(html)
...
def list_videos(channel, cat):
    ...
    filePath = utils.downloadCatalog(
        url_base + cat, 'futura%s.html' % cat.replace('/video/', '').replace('/', ''), False, {}
    )
    html = open(filePath).read().replace('\xe9', 'e').replace('\xe0',
                                                              'a').replace('\n',
                                                                           ' ').replace('\r', '')
