...
def list_shows(channel, folder):
    ...
    for item in globalvar.ordered_channels:
        ...
        if chan_id != 'favourites' and chan_id != 'mostviewed' and chan_id != channel:
            ...
            message = chan_title + str(i) + '-' + str(len(globalvar.channels) - 3) + '-' + str(
                i / (len(globalvar.channels) - 3)
            )
            ...
            shows.append([
                channel, chan_id, chan_title,
                os.path.join(globalvar.MEDIA, chan_id + ".png"), 'shows'
            ])
