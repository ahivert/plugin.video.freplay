    ...
    if folder == 'none':
        ...
        for video in videos:
            ...
            if idSet not in uniqueItem:
                ...
                imgURL = video['AssetSet']['Show']['Image']['ImageRepresentations'][
                    'ImageRepresentation'
                ][0]['url'] + imgFormat
                shows.append([
                    channel, folder + '-' + idSet, video['AssetSet']['title'].encode('utf-8'),
                    imgURL.encode('utf-8'), 'shows'
                ])
...
def list_videos(channel, folderIdSet):
    ...
    for video in videos:
        ...
        idSet = video['AssetSet']['Show']['id']
        if idSet == paramIdSet:
            imgURL = video['Image']['ImageRepresentations']['ImageRepresentation'][0]['url'
                                                                                      ] + imgFormat
            titre = video['AssetSet']['Show']['title'].encode('utf-8') + '-' + video[
                'AssetMetadatas'
            ]['AssetMetadata'][0]['title'].encode('utf-8')
