...
def list_shows(channel, folder):
    ...
    match = re.compile(
        r'<h2><a class="category-link" href="(.*?)"><img class="category-logo" src="(.*?)"> <span>(.*?)</span></a></h2>',
        re.DOTALL
    ).findall(html)
...
def list_videos(channel, folder):
    ...
    filePath = utils.downloadCatalog(
        'http://becurioustv.com' + folder, 'becurious' + folder.replace('/', '') + '.html', False,
        {}
    )
    ...
    match = re.compile(
        r'<div class="video"> <a href="(.*?)">(.*?)<img(.*?)src="(.*?)"(.*?)<h2>(.*?)</h2>',
        re.DOTALL
    ).findall(html)
