        ...
        for prgm in jsonParser['programs']:
            if channel in prgm['channels']:
                if prgm['category'] not in uniqueItem:
                    shows.append([
                        channel, prgm['categorySlug'], prgm['category'].encode('utf-8'), '',
                        'folder'
                    ])
...
def list_videos(channel, id):
    ...
    for vid in jsonParser['videos']:
        if vid['programId'] == id:
            if globalvar.ADDON.getSetting(channel + 'Bonus') == 'true' or vid['videoType'][
                'type'
            ] == 'replay' or vid['videoType']['type'] == 'video':
...
def getVideoURL(channel, media_id):
    ...
    req.add_data((
        ...
        app_name, method, media_id, auth_key, version, hosting_application_name,
        hosting_application_version
    ))
    ...
    if globalvar.ADDON.getSetting('tf1ForceHD') == 'true':
        ...
        m3u8_url = urlparse.urlunsplit((scheme, netloc, path, new_query_string, fragment))
