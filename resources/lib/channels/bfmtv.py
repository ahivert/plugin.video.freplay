...
urlToken = {
    'bfmtv': 'http://api.nextradiotv.com/bfmtv-android/4/',
    'bfmbusiness': 'http://api.nextradiotv.com/bfmbusiness-iphone/3/'
}

urlMenu = {
    'bfmtv': 'http://api.nextradiotv.com/bfmtv-android/4/%s/getMainMenu',
    'bfmbusiness': 'http://api.nextradiotv.com/bfmbusiness-iphone/3/%s/getMainMenu'
}

urlVideoList = {
    'bfmtv':
    'http://api.nextradiotv.com/bfmtv-android/4/%s/getVideosList?count=40&page=1&category=%s',
    'bfmbusiness':
    'http://api.nextradiotv.com/bfmbusiness-iphone/3/%s/getVideosList?count=40&page=1&category=%s'
}

urlVideo = {
    'bfmtv': 'http://api.nextradiotv.com/bfmtv-android/4/%s/getVideo?idVideo=%s&quality=2',
    'bfmbusiness':
    'http://api.nextradiotv.com/bfmbusiness-iphone/3/%s/getVideo?idVideo=%s&quality=2'
}
...
def list_shows(channel, param):
    ...
    for menu in jsonParser['menu']['right']:
        if menu['type'] == 'REPLAY':
            shows.append([
                channel, menu['category'], menu['title'].encode('utf-8'),
                menu['image_url'].encode('utf-8'), 'shows'
            ])
...
def list_videos(channel, show):
    ...
    if show == 'TOUTACTU':
        jsonFile = urllib2.urlopen(
            'http://api.nextradiotv.com/bfmtv-android/4/%s/getLastVideosList?count=40&page=1' %
            (get_token(channel))
        ).read()
    else:
