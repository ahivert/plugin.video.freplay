...
def list_shows(channel, folder):
    ...
    if folder == 'none':
        match = re.compile(
            r'<div class="bottom">(.*?)<div class="titre"><div class="tiret1"><div class="titre_video"(.*?)">(.*?)</div></div><div class="titre_video"(.*?)">(.*?)</div>(.*?)<a href="(.*?)"><img src="(.*?)" alt=',
            re.DOTALL
        ).findall(html)
        if match:
...
def list_videos(channel, show):
    ...
    while another_page == True:
        ...
        filePath = utils.downloadCatalog(
            url_page, channel + '_' + title + '_' + str(current_page) + '.html', False, {}
        )
        html = open(filePath).read()
        ...
        match = re.compile(
            r'<div class="bloc_gauche_image">(.*?)<img src="(.*?)" border="0"(.*?)Date :</span>(.*?)</div>(.*?)Durée : </span>(.*?):(.*?):(.*?)</div>(.*?)<span(.*?)>(.*?)</span>(.*?)>(.*?)</span>(.*?)<a href="(.*?)" class=(.*?)',
            re.DOTALL
        ).findall(html)
