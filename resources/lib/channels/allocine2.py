...
def list_shows(channel, folder):
    ...
    if folder == 'none':
        ...
        if folder == 'ba':
            shows.append([
                channel, 'video/bandes-annonces/|1', 'A ne pas manquer'.encode('utf-8'), '', 'shows'
            ])
            shows.append([
                channel, '/bandes-annonces/plus-recentes/|1', 'Les plus recentes'.encode('utf-8'),
                '', 'shows'
            ])
            shows.append([
                channel, '/bandes-annonces/prochainement/|1', 'Bientot au cinema'.encode('utf-8'),
                '', 'shows'
            ])
        else:
            if 'programme' in folder:
                filePath = utils.downloadCatalog(
                    'http://www.allocine.fr/' + folder,
                    'allocine' + folder.replace('\\', '') + '.html', False, {}
                )
                ...
                match = re.compile(
                    r'<a class="button btn-primary btn-large" href="(.*?)">(.*?)</a>', re.DOTALL
                ).findall(html)
                for url, title in match:
                    shows.append([
                        channel, url + '|1',
                        title.replace("<i class='icon-sign-plus'></i>", ""), '', 'shows'
                    ])

            else:
                cat, page = folder.split('|')
                filePath = utils.downloadCatalog(
                    'http://www.allocine.fr/video/prgcat-' + cat + '/?page=' + page,
                    'allocine' + cat + '-' + page + '.html', False, {}
                )
                html = open(filePath).read().replace('\xe9', 'e').replace('\xe0', 'a').replace(
                    '\n', ' '
                ).replace('\r', '')
                ...
                match = re.compile(
                    r'btn-primary btn-large (.*?)">(.*?)<i class="icon-arrow-(.*?)"></i>', re.DOTALL
                ).findall(html)
                prev = False
                next = False
                ...
                if prev:
                    shows.append([
                        channel, cat + '|' + str(int(page) - 1), '<<Page Precedente', '', 'folder'
                    ])

                match = re.compile(
                    r'<h2 class="title "> <span > <a href="(.*?)">(.*?)</a> </span> </h2>',
                    re.DOTALL
                ).findall(html)
                for url, title in match:
                ...
                if next:
                    shows.append([
                        channel, cat + '|' + str(int(page) + 1), 'Page Suivante>>', '', 'folder'
                    ])
...
def list_videos(channel, folder):
    ...
    videos = []
    filePath = utils.downloadCatalog(
        'http://www.allocine.fr/' + cat + '/?page=' + page, 'allocine' + cat + '-' + page + '.html',
        False, {}
    )
    ...
    match = re.compile(
        r'<div class="layer-link-holder"><a href="/video/player_gen_cmedia=(.*?)&amp;cfilm=(.*?).html" class="layer-link">(.*?)</a></div>',
        re.DOTALL
    ).findall(html)
    if match:
    ...
    match = re.compile(
        r'<h3 class="title "> <span > <a href="/video/video-(.*?)/" itemprop="url">(.*?)</a> </span> </h3>',
        re.DOTALL
    ).findall(html)
    if match:
...
def getVideoURL(channel, idVideo):
    filePath = utils.downloadCatalog(
        'http://www.allocine.fr/ws/AcVisiondataV4.ashx?media=%s' % (idVideo),
        'allocine%s.xml' % (idVideo), False, {}
    )
