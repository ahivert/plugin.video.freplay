    ...
    if folder != 'none':
        for c in jsonParser['categories']:
            if c['id'] == folder:
                for s in c['sub']:
                    shows.append([
                        channel, '/' + c['id'] + '/' + s['id'] + '|1', s['name'], '', 'shows'
                    ])
...
def list_videos(channel, folder):
    ...
    filePath = utils.downloadCatalog(
        'http://www.les-docus.com%s/page/%s/' % (cat, page), 'lesdocuslist.html', True, {}
    )
    ...
    if prev:
        videos.append([
            channel, cat + '|' + str(int(page) - 1), '<<Page Precedente', '', {}, 'shows'
        ])

    match = re.compile(r'<div class="post-header"> <a href="(.*?)" title="(.*?)">',
