            ...
            for program in programs:
                if program['title'].encode('utf-8') == param_show:
                    ...
                    for item in videos_list:
                        url_video_info = '%s/list/%s/%s' % (
                            url_pg_infos, channel_index[channel], item
                        )
                        ...
                        for video in video_infos:
                            try:
                                ...
                                if video_id not in video_done:
                                    ...
                                    infos['Title'] = video['INFOS']['TITRAGE']['TITRE'
                                                                               ].encode('utf-8')
                                    infos['Sub'] = video['INFOS']['TITRAGE']['SOUS_TITRE'
                                                                             ].encode('utf-8')
                                    if infos['Sub'] != "":
                                        infos['Title'
                                              ] = "%s - [I]%s[/I]" % (infos['Title'], infos['Sub'])
                                    infos['Thumb'] = video['MEDIA']['IMAGES']['GRAND'
                                                                              ].encode('utf-8')
                                    video_fanart = video['MEDIA']['IMAGES']['GRAND'].encode('utf-8')
                                    video_name = infos['Title']
                                    videos.append([
                                        channel, video_id, video_name, infos['Thumb'], infos, 'play'
                                    ])
