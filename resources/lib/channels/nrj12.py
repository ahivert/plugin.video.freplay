...
def list_shows(channel, folder):
    ...
    filePath = utils.downloadCatalog(
        'http://www.nrj-play.fr/%s/replay' % channel, channel + '.html', False, {}
    )
    ...
    if folder == 'none':
        match = re.compile(
            r'<li class="subNav-menu-item">(.*?)<a href="(.*?)" class=(.*?)>(.*?)</a>', re.DOTALL
        ).findall(html)
        if match:
        ...
        filePath = utils.downloadCatalog(
            'http://www.nrj-play.fr%s' % (folder), channel + folder + '.html', False, {}
        )
        html = open(filePath).read().replace('\xe9', 'e').replace('\xe0', 'a').replace("\n", "")
        html = ' '.join(html.split())
        print html
        ...
        match = re.compile(
            r'<h2 class="linkProgram-title"> <a href="(.*?)">(.*?)</a> </h2>', re.DOTALL
        ).findall(html)
        print 'rr'
        if match:
...
def list_videos(channel, link):
    ...
    filePath = utils.downloadCatalog(
        'http://www.nrj-play.fr' + link, channel + link.replace('/', '') + '.html', False, {}
    )
    html = open(filePath).read().replace('\xe9', 'e').replace('\xe0', 'a').replace("\n", "")
    html = ' '.join(html.split())
    print html
    ...
    match = re.compile(
        r'<h3 class="thumbnailReplay-title" itemprop="name"> <a href="(.*?)">(.*?)</a> </h3>',
        re.DOTALL
    ).findall(html)
...
def getVideoURL(channel, link):
    ...
    match = re.compile(r'<meta itemprop="contentUrl" content="(.*?)" alt="',
