            ...
            if canPlay(lineupitem):
                if lineupitem['Title'] is not None:
                    if lineupitem['Title'] not in uniqueItem:
                        shows.append([
                            channel, lineupitem['Title'].encode('utf-8'),
                            lineupitem['Title'].encode('utf-8'),
                            lineupitem['ImageUrl'].encode('utf-8'), 'shows'
                        ])
...
def getVideoURL(channel, video_URL):
    filPrgm = urllib2.urlopen(
        'http://ici.tou.tv/presentation%s?v=2&d=ipad&includePartnerTeaser=false' % (video_URL)
    ).read()
    jsonParser = json.loads(filPrgm)
    IdMedia = jsonParser['IdMedia']
    IdMedia = 122445
    filPrgm = urllib2.urlopen(
        'http://api.radio-canada.ca/ValidationMedia/v1/Validation.html?appCode=toutv&idMedia=%s&deviceType=ipad&output=json'
        % (IdMedia)
    ).read()
    print 'http://api.radio-canada.ca/ValidationMedia/v1/Validation.html?appCode=toutv&idMedia=%s&deviceType=ipad&output=json' % (
        IdMedia
    )
    jsonParser = json.loads(filPrgm)
