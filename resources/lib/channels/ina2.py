        ...
        for c in jsonParser['categories']:
            if c['id'] == folder:
                for s in c['sub']:
                    shows.append([
                        channel, c['id'] + '|' + s['id'] + '|0',
                        utils.formatName(s['id'].replace('+', ' ')), '', 'shows'
                    ])
...
def list_videos(channel, url):
    ...
    if id != '0':
        videos.append([
            channel, cat + '|' + sub + '|' + str(int(id) - 24), '<<Page Precedente', '', {}, 'shows'
        ])
    ...
    for url, img, title in match:
        ...
        videos.append([
            channel, idVideo,
            title.encode('utf-8'),
            img.encode('utf-8'), infoLabels, 'play'
        ])
    videos.append([
        channel, cat + '|' + sub + '|' + str(int(id) + 24), 'Page Suivante>>', '', {}, 'shows'
    ])
    return videos
...
def getVideoURL(channel, idVideo):
    filePath = utils.downloadCatalog(
        'https://player.ina.fr/notices/%s.mrss' % (idVideo), 'ina%s.xml' % (idVideo), False, {}
    )
